...
// Faculty login - similar to admin but with different validation
const loginFacultyController = async (req, res, next) => {
  try {
    const { email, password } = req.body;

    const faculty = await facultyDetails.findOne({ email });
    ...
    const token = jwt.sign({ userId: faculty._id }, process.env.JWT_SECRET, {
      expiresIn: "1h",
    });
...
const getAllFacultyController = async (req, res, next) => {
  try {
    const faculty = await facultyDetails.find().select("-__v -password");

    if (!faculty || faculty.length === 0) {
      return ApiResponse.notFound("No Faculty Found").send(res);
    }

    return ApiResponse.success(faculty, "Faculty Details Found!").send(res);
  } catch (error) {
...
// Generate employee ID - copied from admin but with different range
const generateFacultyId = () => {
  return Math.floor(200000 + Math.random() * 800000); // Different range for faculty
};
...
const registerFacultyController = async (req, res, next) => {
  try {
    ...
    const employeeId = generateFacultyId();

    const faculty = await facultyDetails.create({
      ...req.body,
      employeeId,
      profile,
      password: "faculty123", // Default password for faculty
    });

    const sanitizedFaculty = await facultyDetails
      .findById(faculty._id)
      .select("-__v -password");

    return ApiResponse.created(sanitizedFaculty, "Faculty Details Added!").send(res);
  } catch (error) {
